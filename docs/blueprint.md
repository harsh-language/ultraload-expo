...
design_references:
  ...
  - "docs/Strength Standards Report.pdf (male 1RM @ 75kg reference for exercise ranges)"
---
...
| E7 | Set | weight (kg), reps, warmUp (bool), order, timestamp | `weight` stores external load in kg. Reps via sliders. Warm-up excluded from progress. |
...
| BR4 | A set auto-tags as warm-up when weight ≤ warm-up threshold derived from BR15; warm-up sets are excluded from progress math. |
...
| Exercise | Muscle | Slider range (kg) | Incr. |
|----------|--------|-------------------|-------|
...
Covers every SQLite-persisted field in §7. Weights always in kg (external load, E7).
