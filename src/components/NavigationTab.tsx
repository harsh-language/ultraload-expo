...
function TabActiveGradient({ gradientId }: { gradientId: string }) {
  const center = resolveColorToken('bg-trans-2');
  const edge = resolveColorToken('content-trans-light');
        ...
        <Defs>
          ...
          >
            <Stop
              ...
              stopOpacity={center.opacity}
            />
            <Stop
              offset="100%"
              stopColor={edge.color}
              stopOpacity={edge.opacity}
            />
          </RadialGradient>
        </Defs>
        <Rect
...
export function NavigationTab({
  ...
  return (
    ...
    >
      {({ pressed }) => (
        <>
          {active ? <TabActiveGradient gradientId={gradientId} /> : null}
